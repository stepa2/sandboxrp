...
-- fn DB_RegisterSupportedComponent(obj_type: string, comp_name: string, fields: array(string))
local function DB_RegisterSupportedComponent(obj_type, comp_name, fields)
    local q_fields = {}
    for i, field in ipairs(fields) do
        q_fields[i] = "("..SQLEscape(field)..")"
    end
    q_fields = table.concat(q_fields, ", ")

    SQL([[
        INSERT OR IGNORE 
            INTO boxrp_udata_supported_comps (obj_type, comp_name, field_name)
            SELECT {obj_type}, {comp_name}, field_name FROM
                ( VALUES ( {$q_fields} ) )
    ]], {obj_type = obj_type, comp_name = comp_name, q_fields = q_fields})
end

-------- Database - object

-- fn DB_CreateSaveObject(save_set: string, obj_type: string) -> .ObjectId
local function DB_CreateSaveObject(save_set, obj_type)
...
-- fn DB_LoadObject(save_set: string, obj_id: .ObjectId) -> obj_type: string|nil
local function DB_LoadObject(save_set, obj_id)
    local q_result = SQLSingle([[
        SELECT obj.obj_type
            FROM boxrp_udata_obj AS obj
...
-- fn DB_DeleteObjects(save_set: string, obj_ids: array(.ObjectId))
local function DB_DeleteObjects(save_set, obj_ids)
    local q_obj_ids = {}
    ...
    SQL([[
        DELETE FROM boxrp_udata_obj as obj
            WHERE obj.save_set == {save_set} AND obj.obj_id IN ({q_obj_ids})
    ]], {save_set = save_set, q_obj_ids = q_obj_ids})
end

--[[ 
    fn DB_LoadObjectsRecursive(save_set: string, obj_ids: array(.ObjectId), supported_only: bool) -> array({
        obj_id: .ObjectId,
        obj_type: string
    })

]]
local function DB_LoadObjectsRecursive(save_set, obj_ids, supported_only)
    local q_obj_ids = {}
    for i, obj_id in ipairs(obj_ids) do
        q_obj_ids[i] = "("..SQLEscape(obj_id)..")"
    end
...
local function DB_LoadComponents(save_set, obj_id, supported_only)
...
--[[
    ! Wrap this function in transaction to achieve peformance
...
local function DB_SaveComponents(save_set, obj_id, components)
...
local function DB_FindObjectByComponent(save_set, obj_type, comp_name, constraints)
    local q_constraints = {}
